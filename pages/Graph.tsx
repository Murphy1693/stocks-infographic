...
const Graph = ({graphSubscription}: GraphProps) => {
  ...
  useEffect(() => {
    console.log("WINDOW REF", windowRef.current.getBoundingClientRect().width)
    ...
    if (timePeriod === 0) {
      axios.get(`/api/currentPrice?finnhub_symbol=${symbol}`).then((response) => {
        ...
        LineGraph(graphRef.current, response.data.results, {...options, title: response.data.name, yDomain: [response.data.min, (difference + response.data.max)]}, windowRef.current.getBoundingClientRect().width)
      })
    } else {
      axios.get(`/api/closingPrice?alpha_symbol=${symbol}&time=${timePeriods[timePeriod]}`).then((response) => {
        console.log(response)
        setDisplay(response.data.name);
        LineGraph(graphRef.current, response.data.results, {...options, title: response.data.name, yDomain: [0, (1.2 * response.data.max)]}, windowRef.current.getBoundingClientRect().width)
      })
    }
  }, [symbol, timePeriod])

  return <div ref={windowRef} className="flex flex-col pt-4 justify-center md:flex md:flex-row">
